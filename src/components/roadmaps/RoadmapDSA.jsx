...
export default function RoadmapDSA() {
  const navigate = useNavigate();
  const [selectedNode, setSelectedNode] = useState(null);
  
  // Candy Crush style winding path coordinates
  const [nodes, setNodes] = useState([
    { id: 1, name: 'Introduction to DSA', icon: '📚', status: 'completed', xp: 100, coins: 10, x: 50, y: 4, lessons: 12, stars: 3 },
    { id: 2, name: 'Arrays', icon: '📊', status: 'completed', xp: 120, coins: 12, x: 25, y: 10, lessons: 15, stars: 2 },
    { id: 3, name: 'Strings', icon: '📝', status: 'active', xp: 120, coins: 12, x: 75, y: 16, lessons: 14, stars: 0 },
    { id: 4, name: 'Linked Lists', icon: '🔗', status: 'locked', xp: 150, coins: 15, x: 50, y: 22, lessons: 18, stars: 0 },
    { id: 5, name: 'Stacks', icon: '📚', status: 'locked', xp: 140, coins: 14, x: 25, y: 28, lessons: 16, stars: 0, game: true },
    { id: 6, name: 'Queues', icon: '🎫', status: 'locked', xp: 140, coins: 14, x: 70, y: 34, lessons: 16, stars: 0 },
    { id: 7, name: 'Trees', icon: '🌳', status: 'locked', xp: 180, coins: 18, x: 40, y: 40, lessons: 20, stars: 0 },
    { id: 8, name: 'Binary Search Trees', icon: '🌲', status: 'locked', xp: 200, coins: 20, x: 75, y: 46, lessons: 22, stars: 0 },
    { id: 9, name: 'Heaps', icon: '⛰️', status: 'locked', xp: 180, coins: 18, x: 50, y: 52, lessons: 18, stars: 0 },
    { id: 10, name: 'Hashing', icon: '#️⃣', status: 'locked', xp: 160, coins: 16, x: 25, y: 58, lessons: 17, stars: 0, game: true },
    { id: 11, name: 'Graphs', icon: '🕸️', status: 'locked', xp: 200, coins: 20, x: 65, y: 64, lessons: 24, stars: 0 },
    { id: 12, name: 'BFS & DFS', icon: '🔍', status: 'locked', xp: 180, coins: 18, x: 35, y: 70, lessons: 20, stars: 0 },
    { id: 13, name: 'Sorting', icon: '🔢', status: 'locked', xp: 160, coins: 16, x: 70, y: 76, lessons: 22, stars: 0 },
    { id: 14, name: 'Searching', icon: '🔎', status: 'locked', xp: 150, coins: 15, x: 45, y: 82, lessons: 18, stars: 0 },
    { id: 15, name: 'Dynamic Programming', icon: '💡', status: 'locked', xp: 250, coins: 25, x: 25, y: 88, lessons: 28, stars: 0, game: true },
    { id: 16, name: 'Greedy', icon: '💰', status: 'locked', xp: 200, coins: 20, x: 65, y: 94, lessons: 24, stars: 0 },
    { id: 17, name: 'Backtracking', icon: '🔙', status: 'locked', xp: 220, coins: 22, x: 40, y: 100, lessons: 26, stars: 0 },
    { id: 18, name: 'Divide & Conquer', icon: '⚔️', status: 'locked', xp: 200, coins: 20, x: 70, y: 106, lessons: 24, stars: 0 },
    { id: 19, name: 'Bit Manipulation', icon: '🔧', status: 'locked', xp: 180, coins: 18, x: 45, y: 112, lessons: 20, stars: 0 },
    { id: 20, name: 'Final Challenge', icon: '🏆', status: 'locked', xp: 300, coins: 30, x: 50, y: 120, lessons: 30, stars: 0 }
  ]);
  ...
  const handleNodeClick = (node) => {
    if (node.status !== 'locked') {
      ...
      if (node.status === 'active') {
        // Complete current node and unlock next
        setNodes(prevNodes =>
          prevNodes.map((n, idx) => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#0b1020] via-[#1a1f3a] to-[#0f1729]">
      ...
      <div className="max-w-2xl mx-auto relative z-10 px-4 py-6">
        {/* Top Stats Bar - Candy Crush Style */}
        <motion.div
          initial={{ y: -20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          className="flex items-center justify-between mb-6 bg-gradient-to-r from-purple-600 via-pink-600 to-purple-600 rounded-3xl px-6 py-4 shadow-2xl border-4 border-purple-400/30"
        >
          <button
        ...
        <motion.div
          ...
          className="relative bg-gradient-to-b from-gray-900/40 via-purple-900/20 to-gray-900/40 rounded-3xl p-8 backdrop-blur-xl border-4 border-purple-500/30 shadow-2xl"
        ...
        >
          ...
          <motion.div 
            className="absolute top-96 right-10 text-4xl"
          ...
          <motion.div 
            className="absolute bottom-64 left-12 text-3xl"
          ...
          <motion.div 
            className="absolute top-2/3 right-8 text-3xl"
        ...
        <AnimatePresence>
          {selectedNode && (
            <motion.div
              ...
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 20, opacity: 0 }}
              className="mt-6 bg-gradient-to-br from-purple-600 via-pink-600 to-purple-700 rounded-3xl p-6 shadow-2xl border-4 border-purple-400/40"
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-3">
                  ...
                  {selectedNode.status !== 'locked' && (
                    <button
                      className="px-6 py-3 bg-white text-purple-600 font-bold rounded-full shadow-xl hover:scale-105 transition-transform border-2 border-white flex items-center gap-2"
                    >
                      Play ▶
                    </button>
                  )}
